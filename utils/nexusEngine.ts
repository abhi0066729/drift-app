...
function computeSimilarity(a: Note, b: Note): number {
  ...
  // Use Array.from for better TS compatibility
...
export function computeConstellations(notes: Note[]): NexusLayout {
  ...
  // Optimization: Map for fast note lookup
  ...
  // 4. Natural Constellation Bonds — No hard cap, energy-governed
  ...
  // Bond every node to its best local resonant neighbors (uncapped by count)
  nodes.forEach((n1) => {
    if (n1.isDust) return;
    ...
    const targets = nodes
      ...
      .slice(0, 8); // Take best 8 candidates, energy system will prune
  ...
  // 5. Energy System — Find connected components, compute total energy, govern stability
  //    Energy per bond = avg of the two node energies
  //    VIBRATE: constellation energy > threshold → bonds pulse/shimmer
  //    BREAK:   constellation energy >> threshold → weakest bonds snap, nodes re-stabilize

  const VIBRATE_THRESHOLD = 4.5; // ~6 bonds at avg 0.75 energy
  const BREAK_THRESHOLD   = 7.0; // ~10+ bonds — constellation fractures
  ...
  // Sum energy per constellation (component)
  ...
  // Final bond list: exclude broken ones
